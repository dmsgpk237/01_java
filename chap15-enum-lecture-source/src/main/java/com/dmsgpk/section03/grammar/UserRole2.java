...
public enum UserRole2 {
    ...
    //Enum에서는 각각의 요소들이 특정한 값을 가지게 할 수 있다.
    ...
    * Enum 상수의 괄호 안에 넣은 값이 해당 생성자 쪽으로 전달되며,
    * Enum 인스턴스가 생성된다. 생성된 인스턴스는 싱글톤 객체이다.
    *
    * 이처럼 예외상황일 때도 전역예외처리로 어떤 오류상황이 있을지 한 번에 모아서 보고 처리 가능.
    * */
    ...
    // 요소들이 가지는 기본 네임과 정수값 이외의 값을 사용하려 할 때,
