package com.ohgiraffers.api.hair.model.run;
...
public class Application {
    ...
    /*
        ...
        dto 클래스를 설계하고, 객체배열로 변환후에 출력해주세요.
    * */

    // 사용자 미용실 예약 서비스에 필요한 필드 선언
    // 미용실 예약 데이터 가져오기
    // 데이터를 담을 객체 배열 만들기
    // 객체 배열에 데이터 담아서 자르기
    // 객체 배열에 담은 데이터 출력하기
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < temp.length; i++) {
            ...
            // 비어있는곳 null 처리
            LocalDate birthDate = !"".equals(userInfo[5]) ?
                    LocalDate.parse(userInfo[5], DateTimeFormatter.ofPattern("yyyy/MM/dd")) :
                    null;

            LocalDate reservationDate = LocalDate.parse(userInfo[6]);

            LocalTime reservationTime = LocalTime.parse(userInfo[7], DateTimeFormatter.ofPattern("H:m"));

            LocalDateTime createdAt = LocalDateTime.parse(userInfo[8]);

            users[i] = new User(id, firstName, lastName, email, gender, birthDate, reservationDate, reservationTime, createdAt);
