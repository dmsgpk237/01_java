package com.dmsgpk.section05.overloading;
...
public class OverloadingTest {
    ...
    * 이 때, 동일한 기능의 메소드를 매개변수에 따라 다르게 이름을 정의하면 복잡하고 관리하기가 어렵다.
    ...
    * 즉, 시그니처 중 메소드 이름은 동일해야 하기 때문에 파라미터 선언부가 다르게 작성되어야 오버로딩이 성립됨.
    * */
    ...
    * 메소드의 메소드 명과 파라미터 선언부 부분을 시그니처라고 한다. * 'method(int num)' -> 이게 시그니처
    * int num -> 매개변수가 달라야 하는 부분
...
//    } 반환형도 메소드 시그니처에 해당하지 않음.
    ...
    public void test(int num, int num2) {} // 매개변수의 개수에 따라 오버로딩 성립
    ...
    // 나중에 오버라이딩을 배운다. 헷갈리지 않게 개념을 잘 이해하면 좋다.
